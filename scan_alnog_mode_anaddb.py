...
parser.add_argument("-a", "--amplitudes", action="store", type=str, dest="amplstr", default='0.1 0.25 0.5 1', help="Mode shift  amplitude, space separated")
...
#Go through each i mode
    ...
    for i in range(natom):
        print(shiftvec[i])
    for a in args.amplstr.split():
        cartshiftdm=[]
        for i in range(natom):
# multiplyer 25 for backward capability with scan_alnog_mode_abinit.py
            cartshiftdm.append(cartpos[i]-np.array(shiftvec[i])*float(a)*25)

        abinitfn="shiftcell-%.3f.in" % float(a)

        genabinit(abinitfn,args.modenum,basis,natom,typat,znucl,cartshiftdm,'freq = %9.4f cm-1; Delta=%6.4f' % (freqs[args.modenum],float(a)))
